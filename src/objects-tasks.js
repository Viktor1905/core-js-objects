...
function shallowCopy(obj) {
  ...
  const fromEntr = Object.fromEntries(entries);
  const result = Object.assign(fromEntr, {});
  return result;
}
...
function sellTickets(queue) {
  ...
  queue.forEach((entry) => {
    if (entry === 25) {
      ...
      if (box.find((item) => item === 25)) {
        const index = box.findIndex((item) => item === 25);
        box.splice(index, 1);
        box.push(entry);
      } else {
        result = false;
      ...
      if (
        box.findIndex((item) => item === 25) !== -1 &&
        box.reduce((acc, item) => acc + item, 0) >= 75
      ) {
        if (box.findIndex((item) => item === 50)) {
          const index = box.findIndex((item) => item === 25);
          box.splice(index, 1);
          const index2 = box.findIndex((item) => item === 50);
          box.splice(index2, 1);
          box.push(entry);
        } else {
          for (let i = 0; i < 4; i += 1) {
